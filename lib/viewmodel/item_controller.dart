...
class ItemController extends GetxController {
  ...
  void onInit() async {
    super.onInit();
    pagingController.addPageRequestListener((pageKey) => fetchItems(pageKey));
  }
  ...
  Future<void> fetchItems(int pageKey) async {
    ...
    result.when(
      success: (data) {
        ...
        final filteredData = _filterItems(data);
  ...
  List<ItemModel> _filterItems(List<ItemModel> items) {
    ...
    return items.where((item) {
      ...
      final titleMatches = item.title?.contains(searchQuery.value) ?? false;
      return titleMatches;
    }).toList();
  }
